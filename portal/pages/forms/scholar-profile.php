...
if ($stud_id > 0) {
    ...
    $query = "SELECT * FROM tbl_students WHERE stud_id = '$stud_id'";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
        ...
        $pdf->Cell(95, 9, 'NORTHRIDGE INSTITUTE OF BUSINESS AND TECHNOLOGY', 0, 1, 'C');

        $pdf->Ln(3);
        $pdf->SetTextColor(0, 0, 0);
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(200, 8, 'ADMIN(?) OFFICE', 0, 1, 'C'); // GUIDANCE OFFICE
        ...
        $pdf->Cell(20, 4, $row['lastname'], 0, 0); //data - Last Name
        $pdf->Cell(48, 4, '', 0, 0); // space
        $pdf->Cell(45, 4, $row['firstname'], 0, 0); // data
        $pdf->Cell(20, 4, '', 0, 0); // space
        $pdf->Cell(45, 4, $row['middlename'], 0, 1); // data
